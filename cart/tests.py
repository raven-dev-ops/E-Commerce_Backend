...
class CartModelTest(TestCase):
    ...
    def test_cart_item_creation(self):
        ...
        self.assertEqual(str(item.product.id), str(self.product.id))
...
class CartAPITestCase(TestCase):
    ...
    def test_add_item_to_cart(self, mock_get_cart, mock_objects):
        ...
        cart_item = CartItem(cart=self.cart, product=self.product, quantity=1)
        mock_objects.get_or_create.return_value = (cart_item, True)
        url = reverse('cart')
        response = self.client.post(url, {'product_id': str(self.product._id), 'quantity': 1}, format='json')
    ...
    def test_update_item_quantity(self, mock_get_cart, mock_objects):
        ...
        url = reverse('cart')
        response = self.client.put(url, {'product_id': str(self.product._id), 'quantity': 3}, format='json')
    ...
    def test_update_item_invalid_quantity(self, mock_get_cart):
        mock_get_cart.return_value = self.cart
        url = reverse('cart')
        response = self.client.put(url, {'product_id': str(self.product._id), 'quantity': 0}, format='json')
        self.assertEqual(response.status_code, 400)
    ...
    def test_delete_item(self, mock_get_cart, mock_objects):
        ...
        mock_objects.get.return_value = cart_item
        url = reverse('cart')
        response = self.client.delete(url, {'product_id': str(self.product._id)}, format='json')
